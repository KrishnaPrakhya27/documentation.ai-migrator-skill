...
export interface ScopeDecisions { excluded: ScopeExclusion[]; substituted: ScopeSubstitution[]; helpSystems: HelpSystemDecision[]; differences: AcceptedDifference[] }
...
export function readScopeDecisions(workspace: string): ScopeDecisions {
  const path = scopeDecisionsPath(workspace);
  if (!existsSync(path)) return { excluded: [], substituted: [], helpSystems: [], differences: [] };
  ...
  return { excluded, substituted, helpSystems, differences };
}
